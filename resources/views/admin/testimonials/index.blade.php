...
@section('scripts')
    <script type="text/javascript">
        ...
        function createUpdateModal(id) {
            ...
            $('#image').hide();
            $('#heading').html('New Testimonial');
            $('#createUpdateForm')[0].reset();

            if (id != 0) {
                $('#heading').html('Update Testimonial');
                $('#name').val($('#name_' + id).html());
                $('#designation').val($('#designation_' + id).html());
                $('#quote').val($('#quote_' + id).html());
                $('#image').show().attr("src", $('#image_' + id).html());
            }
        }

        function createUpdatePost() {
            var url = "{{ route('testimonial.create.update') }}";
