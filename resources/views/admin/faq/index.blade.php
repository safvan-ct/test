...
@section('scripts')
    <script type="text/javascript">
        ...
        function createUpdateModal(id) {
            ...
            $('#heading').html('New Faq');
            $('#createUpdateForm')[0].reset();

            if (id != 0) {
                $('#heading').html('Update Faq');
                $('#product_id').val($('#product_id_' + id).html());
                $('#question').val($('#question_' + id).html());
                $('#answer').val($('#answer_' + id).html());
            }
        }

        function createUpdatePost() {
            var url = "{{ route('faq.create.update') }}";
